...
-(id)initWithFrame:(CGRect)frame byClickTag:(NSInteger)clickTag appendArray:(NSArray *)appendArray
{
    ...
    if (self) {
        ...
        _pageControl.currentPage = clickTag - imageTag;
...
-(void)initScrollerView:(NSArray *)appendArray
{
    if (!_scrollView) {
        ...
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.self_frame.size.height - 50, self.self_frame.size.width, 50)];
        _pageControl.numberOfPages = appendArray.count;
...
-(void)initImageView:(NSArray *)appendArray
{
    for (int i = 0; i < [appendArray count]; i++) {
        ...
        {
            //img = [UIImage imageWithCGImage:(__bridge CGImageRef _Nonnull)(appendArray[i])];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{    //图片滑动后原来的图片大小改变原样
    ...
    _pageControl.currentPage = self.page;
